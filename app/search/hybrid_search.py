...
def hybrid_search(query: SearchQuery) -> tuple[list[dict], int]:
    ...
    ce_rerank=True 时对 RRF 融合结果前 TOP_N 做 CrossEncoder 精排前缀
    （复用 app/search/rerank.py 降级链）；无关键词（纯筛选/浏览全部）不精排。
    """
        ...
        if dist_map:
            ...
            with get_db() as conn:
                ...
                for col, vals in (
                    ...
                    ("s.dim1_industry", query.dim1_industry),
