...
## T9 — 规则 label 解耦的四项已修 + 一项余留（2026-09-20）
...
- **What（余项）**：**规则页仍无「按标签分组」视图**。当前「同一标签多关键词」靠多条规则共享同一 label 表达（dim6 `钢筋` 已由 4 条规则产出），列表是平铺 pattern、按维度筛选。若要「一个标签下列出全部关键词、可就地增删」的编辑体验，需新增分组视图。
- **Why**：平铺视图下，判断「某标签由哪些词支撑」「某词属于哪个标签」必须人工扫表，标签与词面的对应关系不直观——这正是「规则页看起来只有一堆词」这一观感的残留部分（前 4 项已消除「看不出标签」的问题，但未消除「看不出标签↔词面的分组结构」）。
- **Blocked by**：无；纯 UI 增强，视实际维护频率决定是否值得做。
- **Status（2026-09-20）**：登记，未实施。
