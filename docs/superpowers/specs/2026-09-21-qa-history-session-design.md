...
[本次问题]                          ~15 tok
────────────────────────────────────────────
合计上界                            ≈7,060 tok（**条件**：最新一轮自身不超预算，见下）
```
...
> ⚠️ **最新一轮无条件保留**（评审 D15 定，Task 6 实测确认）：丢弃循环是
> `if kept and used + cost > budget: break` —— 即**至少保留一轮**。因此当最新一轮自身
> 就超过 `qa.token.max_history_tokens` 时，历史段会突破该预算（上述「合计上界」
> 只在最新一轮放得下的前提下成立）。
>
> 这是**有意取舍**：历史段一轮都没有时多轮对话即失去意义，而最新一轮恰恰是
> 最相关的一轮。若将来要严格守上界，需改为「对该轮改用摘要或丢弃」——但那会
> 牺牲多轮连贯性，不宜轻改。
>
> 🔴 **但默认值 800 很可能偏小，这不是"极端情况"而是常见情况**（Task 6 复核以算术指出）：
> `estimate_tokens = ceil(len/2)`，故**最新一轮 Q+A 超过约 1590 字符，历史段就恒为 1 轮**，
> `history.max_turns = 6` 形同虚设。带条文引用、2~3 段的普通回答轻易超过 1600 字符。
>
> **当前不拍板改默认值**——真实答案长度分布没有测量数据，改了等于猜。正确次序是
> **先可观测、再调参**：Task 8 需记录每轮实际历史段 token（超预算时落日志/指标），
> 参数已是 0~4000 且热生效，届时按真实分布调整。注意 `build_history` 是纯函数（无 IO），
> **超预算这件事本身不会留下任何痕迹**——可见性必须由调用方提供。
...
- `filtered_out: int` —— **不带分类维度**重新检索时的全局命中数。
  注意它**不是**「被筛掉的条数」，而是「放宽后能拿到多少」——这正是提示文案里那个 N。
  前端判据是 `> 0` 才提示放宽。**取 0 有两种成因**：① 未触发诊断检索（无分类筛选 / 候选已够）；
  ② 触发了但全局命中也是 0（此时放宽也救不回来，故不提示是**正确行为**）。
  （实现修订 2026-09-23：原文写「0 表示未触发诊断检索」，与实现不符，已改正。）
- `effective_filters: dict` —— 本轮实际生效的筛选（分类维度 + 状态 + 前言放行），
  供 §4.3 的可见性提示与随消息落库的追溯（D5）。
  注意「前言放行」只记**用户显式勾选**（复选框），**不记**问题文本触发的兜底——
  兜底是问题文本的确定性函数、而问题原文本身已落库，故回看时可由问题文本**重建**，不构成信息丢失。
